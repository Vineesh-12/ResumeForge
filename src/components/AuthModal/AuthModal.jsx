...
export default function AuthModal() {
  ...
  // Forgot password OTP sub-state
  const [forgotStep, setForgotStep] = useState(1) // 1: Send OTP, 2: Enter OTP + New Pwd
  ...
  const handleSendForgotOtp = async (e) => {
    ...
    try {
      ...
      dispatch({
        type: 'SET_TOAST',
        payload: {
          message: `Security recovery code sent to ${cleanEmail}. Check your inbox.`,
          type: 'success'
        }
      })
    } catch (err) {
  ...
  // Confirm Reset with OTP in modal
  const handleConfirmForgotOtp = async (e) => {
    e.preventDefault()
    setError('')

    if (!forgotEnteredOtp || forgotEnteredOtp.trim().length !== 6) {
      setError('Please enter the 6-digit code sent to your email.')
      return
    }
    if (forgotEnteredOtp.trim() !== forgotGeneratedOtp.trim()) {
      setError('Invalid 6-digit recovery code. Please check your email.')
      return
    }
    ...
    setIsLoading(true)
    try {
      // First sign in or update password
      await updateUserAccountPassword(forgotNewPassword)
      dispatch({
        type: 'SET_TOAST',
        payload: {
      ...
      // If user isn't currently signed in, sign them in with new password
      dispatch({
        type: 'SET_TOAST',
        payload: {
          message: 'Verification verified! Please sign in with your credentials.',
          type: 'success'
        }
      })
      setTab('signin')
          ...
          <div className="modal-title-group">
            ...
            <div>
              ...
              <p className="text-xs text-muted">
                {tab === 'forgot'
                  ? 'Reset your password with a 6-digit email code'
          ...
          <div className="auth-form animate-fade-in">
            {forgotStep === 1 ? (
              <form onSubmit={handleSendForgotOtp}>
                ...
                >
                  <Send size={15} />
                  <span>{isLoading ? 'Sending...' : 'Send 6-Digit Recovery Code'}</span>
                </button>
              ...
              <form onSubmit={handleConfirmForgotOtp}>
                <p className="text-xs text-secondary" style={{ textAlign: 'center', marginBottom: 'var(--space-3)' }}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="• • • • • •"
                    className="input-control"
                    style={{ textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: '1.3rem', fontWeight: 800, letterSpacing: '0.3em' }}
                    value={forgotEnteredOtp}
                    onChange={(e) => setForgotEnteredOtp(e.target.value.replace(/[^0-9]/g, ''))}
                    required
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLoading || forgotEnteredOtp.length !== 6 || !forgotNewPassword}
                  className="btn btn-primary btn-auth-submit"
                  style={{ marginTop: 'var(--space-3)' }}
                >
                  <Check size={15} />
                  <span>{isLoading ? 'Resetting...' : 'Reset & Save Password'}</span>
                </button>
